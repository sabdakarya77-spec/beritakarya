...
export default function Navbar({
  ...
  return (
    <header className={cn(
      ...
      isArticlePage
        ? "border-black/5 bg-[color:rgb(var(--brand-surface-rgb)/0.88)] backdrop-blur-xl shadow-card dark:border-white/5 dark:bg-[rgba(2,6,23,0.84)]"
      ...
      <AnimatePresence initial={false}>
        {!isCollapsed && (
          ...
          >
            <Container className="flex h-9 items-center justify-between gap-4 text-xs font-medium text-brand-text-muted sm:h-10 sm:text-xxs">
              <div className="flex min-w-0 items-center gap-4">
                <div className="min-w-0 truncate">
                  {isArticlePage ? (
                    <span className="truncate text-xs normal-case tracking-[0.03em] text-brand-text-muted sm:text-xxs">
                      {articleTopDate}
                    </span>
                  ) : (
                    <DateTimeWeather />
            ...
            <Container className={cn(
              ...
              >
                <Link href={`/${activeSite}`} className="flex max-w-full flex-col items-center group">
                  ...
                  <span className="mt-1 max-w-[8rem] text-center text-xs font-medium tracking-[0.05em] text-brand-text-muted transition-all sm:max-w-[240px] sm:text-xs sm:tracking-[0.08em]">
                    Nusantara Berbicara 
                  </span>
                </Link>
              </motion.div>
              ...
              <div className={cn("flex min-w-0 items-center justify-end gap-1 sm:gap-2 md:gap-3", isArticlePage && "md:gap-2.5")}>
                ...
                {user ? (
                  <div className="relative hidden sm:block">
                    ...
                    >
                      <div className="flex h-6 w-6 items-center justify-center rounded-full bg-brand-red text-xs font-bold text-white">
                        {user.name.charAt(0).toUpperCase()}
                      </div>
                      <span className="hidden max-w-[88px] truncate text-xxs font-semibold md:inline">
                        {user.name.split(' ')[0]}
                      </span>
                    </button>
                    ...
                    <AnimatePresence>
                      {isProfileOpen && (
                        <motion.div
                          ...
                          animate={{ opacity: 1, y: 0 }}
                          exit={{ opacity: 0, y: 10 }}
                          className="absolute right-0 z-50 mt-2 w-52 overflow-hidden rounded-card border border-black/5 bg-white shadow-modal dark:border-white/5 dark:bg-slate-900"
                        >
                          ...
                          <div className="p-2">
                            {['superadmin', 'wapimred', 'reporter', 'kontributor'].includes(user.role) && (
                              <Link 
                                ...
                                className="block rounded-button px-3 py-2 font-display text-xxs font-bold uppercase tracking-eyebrow text-gray-600 transition-colors hover:bg-gray-50 hover:text-brand-red dark:text-gray-300 dark:hover:bg-white/5"
                            ...
                            <button 
                              ...
                              className="mt-1 w-full rounded-button px-3 py-2 text-left font-display text-xxs font-bold uppercase tracking-eyebrow text-brand-red transition-colors hover:bg-red-50 dark:hover:bg-red-500/10"
                            >
                              Keluar
                            </button>
                          </div>
                  ...
                  >
                    ...
                    <span className="hidden text-xxs font-semibold md:inline">Masuk</span>
                  </Link>
                )}
      ...
      <div className="hidden border-t border-black/5 dark:border-white/5 md:block">
        <Container className={cn(
          "relative z-40 hidden items-center justify-center text-xs font-medium tracking-[0.04em] text-brand-text-muted md:flex lg:text-xxs",
          isCollapsed
            ? "h-10 gap-3.5"
            : "h-12 gap-5"
        )}>
            ...
            >
              ...
              >
                ...
                {cat.slug === 'tersimpan' && savedArticlesCount > 0 && (
                  <span className="inline-flex min-w-5 items-center justify-center rounded-full bg-brand-red px-1.5 py-0.5 text-xs font-black tracking-normal text-white">
                    {savedArticlesCount}
                  </span>
                )}
                {isActive && (
              ...
              <AnimatePresence>
                {hoveredCategory === cat.name && hasSub && (
                  <motion.div
                    ...
                    className="absolute left-1/2 top-full z-50 mt-1 flex min-w-[220px] -translate-x-1/2 flex-col gap-0.5 rounded-card border border-black/5 bg-white/95 p-2 shadow-card backdrop-blur-md dark:border-white/10 dark:bg-slate-900/95 dark:shadow-card"
                  >
                    {cat.subCategories?.map((sub) => {
                      ...
                      return (
                        <button
                          ...
                          className={cn(
                            "group/sub flex items-center justify-between rounded-button px-4 py-2 text-left font-display text-xs font-medium tracking-[0.02em] transition-colors hover:bg-gray-50 dark:hover:bg-white/5 lg:text-xxs",
        ...
        <Container className="md:hidden">
          ...
          return (
            <button
              ...
              onClick={() => handleCategoryClick(cat.slug)}
              className={cn(
                "flex shrink-0 items-center gap-1 whitespace-nowrap rounded-full border px-2.5 py-1.5 text-xxs font-medium transition-all",
                        isCollapsed && "px-2.5 py-1 text-xs",
                isActive
            ...
            >
              ...
              <span>{cat.name}</span>
              {cat.slug === 'tersimpan' && savedArticlesCount > 0 && (
                <span className="inline-flex min-w-4 items-center justify-center rounded-full bg-brand-red px-1 py-0.5 text-xs font-black tracking-normal text-white">
                  {savedArticlesCount}
                </span>
              )}
            </button>
                  ...
                  {activeParent.subCategories.map((sub) => {
                    ...
                    return (
                      <button
                        ...
                        className={cn(
                          "shrink-0 whitespace-nowrap rounded-button px-2.5 py-1 text-xs font-medium transition-all",
                          isCollapsed && "px-2 py-1 text-xxs",
                          isSubActive
