'use client'

import React, { useState, useCallback } from 'react'
import type { Editor } from '@tiptap/react'

export interface ImageToolbarProps {
  editor: Editor
  onImageUpload?: (file: File) => Promise<string>
}

export function ImageToolbar({ editor, onImageUpload }: ImageToolbarProps) {
  const [isUploading, setIsUploading] = useState(false)
  const [error, setError] = useState<string | null>(null)

  const handleFileSelect = useCallback(async (event: React.ChangeEvent<HTMLInputElement>) => {
    const file = event.target.files?.[0]
    if (!file || !onImageUpload) return

    setIsUploading(true)
    setError(null)

    try {
      const url = await onImageUpload(file)
      editor.chain().focus().setImage({ src: url }).run()
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Gagal mengunggah gambar')
    } finally {
      setIsUploading(false)
    }
  }, [editor, onImageUpload])

  const handleAltChange = useCallback((alt: string) => {
    editor.chain().focus().updateAttributes('image', { alt }).run()
  }, [editor])

  const handleDelete = useCallback(() => {
    editor.chain().focus().deleteNode('image').run()
  }, [editor])

  return (
    <div className="flex flex-col gap-3 p-3 bg-white rounded-lg shadow-lg border min-w-[280px]">
      <div className="text-sm font-medium text-gray-700">Image Properties</div>
      <div>
        <label className="block text-xs text-gray-500 mb-1">Upload Gambar</label>
        <input type="file" accept="image/*" onChange={handleFileSelect} disabled={isUploading} className="w-full text-sm border rounded p-1" />
        {isUploading && <span className="text-xs text-blue-500">Mengunggah...</span>}
      </div>
      <div>
        <label className="block text-xs text-gray-500 mb-1">Alt Text</label>
        <input type="text" onChange={(e) => handleAltChange(e.target.value)} placeholder="Deskripsi gambar" className="w-full text-sm border rounded p-1.5" />
      </div>
      <button type="button" onClick={handleDelete} className="w-full px-3 py-2 bg-red-50 text-red-600 rounded hover:bg-red-100 text-sm">Hapus Gambar</button>
      {error && <div className="text-xs text-red-500 mt-1">{error}</div>}
    </div>
  )
}

export default ImageToolbar