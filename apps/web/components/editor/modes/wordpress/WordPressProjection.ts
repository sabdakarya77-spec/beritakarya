...
import { updateBlockData, replaceBlock } from '../../core/editorCommands'
...
export function projectBlocksToWordPressFlow(blocks: Block[]): WordPressFlow {
  ...
  for (const block of blocks) {
    const isNonText = !isTextBlock(block.type)
    if (isNonText && lastWasNonText) interleaveCount++
    lastWasNonText = isNonText
  }
...
export function projectParsedToBlocks(
  ...
  // Map: blockId -> updated data
  const updates = new Map<string, { content?: string; items?: string[]; level?: number }>()
  ...
  for (const node of parsedNodes) {
    ...
    if (!targetId) {
      // No matching block — we'd need to insert, but that's handled externally
      textIdx++
      continue
    }
