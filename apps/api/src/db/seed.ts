...
async function main() {
  ...
  const categoriesData = [
    { name: 'Nasional', slug: 'nasional', description: 'Kabar terkini dan kebijakan penting di tingkat nasional.' },
    { name: 'Politik', slug: 'politik', description: 'Dinamika politik tanah air, legislatif, dan eksekutif.' },
  ...
  for (const cat of categoriesData) {
    const createdCat = await prisma.category.upsert({
      where: { id: cat.slug }, // using slug as ID for predictable seeding
      update: {
        name: cat.name,
        slug: cat.slug,
  ...
  const mockArticles = [
    ...
    },
    {
      ...
      categoryId: categoriesMap['hukum'],
